...
Most of these are Windows utility scripts (PowerShell + Batch, no installs required). One is a Python script that drives a real browser session — see its own README for setup.
...
- **Python scripts**: install the folder's `requirements.txt`, then run the `.py` file directly.
